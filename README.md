...
- `"hidden": true` を付けた作品は一覧（トップ / Works）と詳細ページの生成対象から外れます。データは残るのでフラグを外すだけで再表示できます（一時的に公開停止したいときに使います）。
...
- mock画像をライト/ダークで切り替えたい作品は、各画像（`desktop` / `mobile`）の `src` にライト用画像、`srcDark` にダーク用画像を指定します。`src` が既定（ライト）表示で、`assets/js/theme-mock.js` が現在のテーマに合わせて差し替えます。`srcDark` が無い画像は従来どおりテーマで変化しません。
